...
program
  .version(version)
  .description('SEAD Method installer - Universal AI agent framework for any domain');

program
  .command('install')
  .description('Install SEAD Method agents and tools')
  .option('-f, --full', 'Install complete SEAD Method')
  .option('-x, --expansion-only', 'Install only expansion packs (no sead-core)')
...
program
  ...
  .description('Check for SEAD Update')
  .action(async () => {
...
╚═════╝ ╚═╝     ╚═╝╚═╝  ╚═╝╚═════╝       ╚═╝     ╚═╝╚══════╝   ╚═╝   ╚═╝  ╚═╝ ╚═════╝ ╚═════╝ 
  ...
  const { directory } = await inquirer.prompt([
    {
      type: 'input',
      name: 'directory',
      message: 'Enter the full path to your project directory where SEAD should be installed:',
  ...
  const coreConfigPath = path.join(__dirname, '..', '..', '..', 'sead-core', 'core-config.yaml');
  ...
  // Process selections
  answers.installType = selectedItems.includes('sead-core') ? 'full' : 'expansion-only';
  answers.expansionPacks = selectedItems.filter((item) => item !== 'sead-core');

  // Ask sharding questions if installing SEAD core
  if (selectedItems.includes('sead-core')) {
  ...
  if (ides.includes('github-copilot')) {
    ...
    console.log(
      chalk.dim('SEAD works best with specific VS Code settings for optimal agent experience.\n'),
    );
  ...
  if (ides.includes('auggie-cli')) {
    ...
    console.log(chalk.dim('Choose where to install SEAD agents for Auggie CLI access.\n'));
