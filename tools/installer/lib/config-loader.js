...
class ConfigLoader {
  ...
  async getAvailableAgents() {
    const agentsDir = path.join(this.getSeadCorePath(), 'agents');
          ...
          try {
            ...
            if (yamlContentText) {
              ...
              agents.push({
                ...
                file: `sead-core/agents/${entry.name}`,
  ...
  async getAvailableExpansionPacks() {
    const expansionPacksDir = path.join(this.getSeadCorePath(), '..', 'expansion-packs');
        ...
        if (entry.isDirectory() && !entry.name.startsWith('.')) {
          ...
          try {
            ...
            expansionPacks.push({
              ...
              author: config.author || 'SEAD Team',
              packPath: packPath,
  ...
  async getAgentDependencies(agentId) {
    ...
    for (const resource of agentDeps.resources) {
      const filePath = `.sead-core/${resource.type}/${resource.id}.md`;
  ...
  getSeadCorePath() {
    // Get the path to sead-core relative to the installer (now under tools)
    return path.join(__dirname, '..', '..', '..', 'sead-core');
  }
  ...
  getAgentPath(agentId) {
    return path.join(this.getSeadCorePath(), 'agents', `${agentId}.md`);
  }

  async getAvailableTeams() {
    const teamsDir = path.join(this.getSeadCorePath(), 'agent-teams');
  ...
  getTeamPath(teamId) {
    return path.join(this.getSeadCorePath(), 'agent-teams', `${teamId}.yaml`);
  }
  ...
  async getTeamDependencies(teamId) {
    ...
    try {
      ...
      depPaths.push(`.sead-core/agent-teams/${teamId}.yaml`);

      // Add all agents
      for (const agent of teamDeps.agents) {
        const filePath = `.sead-core/agents/${agent.id}.md`;
        if (!depPaths.includes(filePath)) {
          depPaths.push(filePath);
        }
      }
      ...
      for (const resource of teamDeps.resources) {
        const filePath = `.sead-core/${resource.type}/${resource.id}.${resource.type === 'workflows' ? 'yaml' : 'md'}`;
        if (!depPaths.includes(filePath)) {
          depPaths.push(filePath);
        }
      }
