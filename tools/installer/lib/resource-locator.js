...
class ResourceLocator {
  constructor() {
    ...
    this._seadCorePath = null;
    this._expansionPacksPath = null;
  }

  /**
   * Get the base path for sead-core
   */
  getSeadCorePath() {
    if (!this._seadCorePath) {
      this._seadCorePath = path.join(__dirname, '../../../sead-core');
    }
    return this._seadCorePath;
  }
  ...
  async findFiles(pattern, options = {}) {
    ...
    // Cache for 5 minutes
    this._globCache.set(cacheKey, result);
  ...
  async getAgentPath(agentId) {
    ...
    // Check in sead-core
    let agentPath = path.join(this.getSeadCorePath(), 'agents', `${agentId}.md`);
  ...
  async getAvailableAgents() {
    ...
    // Get agents from sead-core
    const coreDir = this.getSeadCorePath();
    const coreAgents = await this.findFiles('agents/*.md', {
      cwd: coreDir,
    });

    for (const agentFile of coreAgents) {
      const content = await fs.readFile(path.join(this.getSeadCorePath(), agentFile), 'utf8');
  ...
  async getTeamConfig(teamId) {
    ...
    const teamPath = path.join(this.getSeadCorePath(), 'agent-teams', `${teamId}.yaml`);
      ...
      for (const [type, deps] of Object.entries(dependencies)) {
        if (Array.isArray(deps)) {
          ...
          for (const dep of deps) {
            allDeps.push(`.sead-core/${type}/${dep}`);
          }
        }
      }
