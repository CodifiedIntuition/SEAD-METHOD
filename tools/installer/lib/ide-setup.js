...
class IdeSetup extends BaseIdeSetup {
  ...
  async setupCodex(installDir, selectedAgent, options) {
    ...
    // Inject/update a SEAD section with guidance, directory, and details.
    const filePath = path.join(installDir, 'AGENTS.md');
    const startMarker = '<!-- BEGIN: SEAD-AGENTS -->';
    const endMarker = '<!-- END: SEAD-AGENTS -->';

    const agents = selectedAgent ? [selectedAgent] : await this.getAllAgentIds(installDir);
    const tasks = await this.getAllTaskIds(installDir);

    // Build SEAD section content
    let section = '';
    section += `${startMarker}\n`;
    section += `# SEAD-METHOD Agents and Tasks\n\n`;
    section += `This section is auto-generated by SEAD-METHOD for Codex. Codex merges this AGENTS.md into context.\n\n`;
    ...
    section += `- Commit \`.sead-core\` and this \`AGENTS.md\` file to your repo so Codex (Web/CLI) can read full agent definitions.\n`;
    section += `- Refresh this section after agent updates: \`npx sead-method install -f -i codex\`.\n\n`;

    section += `### Helpful Commands\n\n`;
    section += `- List agents: \`npx sead-method list:agents\`\n`;
    section += `- Reinstall SEAD core and regenerate AGENTS.md: \`npx sead-method install -f -i codex\`\n`;
    section += `- Validate configuration: \`npx sead-method validate\`\n\n`;
    ...
    if (await fileManager.pathExists(filePath)) {
      ...
      if (existing.includes(startMarker) && existing.includes(endMarker)) {
        ...
        // Append SEAD block to existing file
        finalContent = existing.trimEnd() + `\n\n` + section;
      }
    } else {
      // Create fresh AGENTS.md with a small header and SEAD block
    ...
    console.log(
      chalk.dim(
        'Codex reads AGENTS.md automatically. Run `codex` in this project to use SEAD agents.',
      ),
    );
    ...
    try {
      ...
      if (await fileManager.pathExists(pkgPath)) {
        ...
        if (!updated['sead:refresh']) updated['sead:refresh'] = 'sead-method install -f -i codex';
        if (!updated['sead:list']) updated['sead:list'] = 'sead-method list:agents';
        if (!updated['sead:validate']) updated['sead:validate'] = 'sead-method validate';
        ...
        if (changed) {
          ...
          console.log(chalk.green('✓ Added npm scripts: sead:refresh, sead:list, sead:validate'));
        }
      }
    } catch {
      console.log(
    ...
    try {
      ...
      const ignoreLines = ['# SEAD (local only)', '.sead-core/', '.sead-*/'];
      ...
      if (options.webEnabled) {
        if (exists) {
          ...
          // Remove lines that ignore SEAD dot-folders
          const updated = gi
            .split(/\r?\n/)
            .filter((l) => !/^\s*\.sead-core\/?\s*$/.test(l) && !/^\s*\.sead-\*\/?\s*$/.test(l))
            .join('\n');
          if (updated !== gi) {
            await fileManager.writeFile(gitignorePath, updated.trimEnd() + '\n');
            console.log(chalk.green('✓ Updated .gitignore to include .sead-core in commits'));
          }
        }
      } else {
        // Local-only: add ignores if missing
        let base = exists ? await fileManager.readFile(gitignorePath) : '';
        const haveCore = base.includes('.sead-core/');
        const haveStar = base.includes('.sead-*/');
        if (!haveCore || !haveStar) {
          ...
          console.log(chalk.green('✓ Added .sead-core/* to .gitignore for local-only Codex setup'));
        }
      }
    } catch {
  ...
  async getAllAgentIds(installDir) {
    ...
    // Check core agents in .sead-core or root
    let agentsDir = path.join(installDir, '.sead-core', 'agents');
  ...
  async getCoreAgentIds(installDir) {
    ...
    // Check core agents in .sead-core or root only
    let agentsDir = path.join(installDir, '.sead-core', 'agents');
    if (!(await fileManager.pathExists(agentsDir))) {
      agentsDir = path.join(installDir, 'sead-core', 'agents');
    }
  ...
  async getCoreTaskIds(installDir) {
    ...
    // Check core tasks in .sead-core or root only
    let tasksDir = path.join(installDir, '.sead-core', 'tasks');
    if (!(await fileManager.pathExists(tasksDir))) {
      tasksDir = path.join(installDir, 'sead-core', 'tasks');
    }
  ...
  async getAllTaskIds(installDir) {
    ...
    // Check core tasks in .sead-core or root
    let tasksDir = path.join(installDir, '.sead-core', 'tasks');
    if (!(await fileManager.pathExists(tasksDir))) {
      tasksDir = path.join(installDir, 'sead-core', 'tasks');
    }
  ...
  async findTaskPath(taskId, installDir) {
    ...
    const possiblePaths = [
      path.join(installDir, '.sead-core', 'tasks', `${taskId}.md`),
      path.join(installDir, 'sead-core', 'tasks', `${taskId}.md`),
  ...
  async getCoreSlashPrefix(installDir) {
    try {
      ...
      if (!(await fileManager.pathExists(coreConfigPath))) {
        // Try sead-core directory
        const altConfigPath = path.join(installDir, 'sead-core', 'core-config.yaml');
        if (await fileManager.pathExists(altConfigPath)) {
          const configContent = await fileManager.readFile(altConfigPath);
          const config = yaml.load(configContent);
          return config.slashPrefix || 'SEAD';
        }
        return 'SEAD'; // fallback
      }
      ...
      console.warn(`Failed to read core slashPrefix, using default 'SEAD': ${error.message}`);
      return 'SEAD';
    }
  }
  ...
  async getInstalledExpansionPacks(installDir) {
    ...
    const glob = require('glob');
    const dotExpansions = glob.sync('.sead-*', { cwd: installDir });
  ...
  async setupRoo(installDir, selectedAgent) {
    ...
    for (const agentId of agents) {
      ...
      // Check both with and without sead- prefix to handle both cases
      const checkSlug = agentId.startsWith('sead-') ? agentId : `sead-${agentId}`;
      ...
      if (agentPath) {
        ...
        if (yamlMatch) {
          ...
          // Avoid double "sead-" prefix for agents that already have it
          const slug = agentId.startsWith('sead-') ? agentId : `sead-${agentId}`;
          ...
          console.log(chalk.green(`✓ Added mode: sead-${agentId} (${icon} ${title})`));
        }
      }
    }
  ...
  async setupCline(installDir, selectedAgent) {
    ...
    for (const agentId of agents) {
      ...
      if (agentPath) {
        ...
        mdContent += `- Always maintain consistency with project documentation in .sead-core/\n`;
...
`;
    ...
    console.log(chalk.dim(`You can now find the SEAD agents in the Chat view's mode selector.`));
  ...
  async configureVsCodeSettings(installDir, spinner, preConfiguredSettings = null) {
    ...
    if (await fileManager.pathExists(settingsPath)) {
      try {
        ...
        console.log(chalk.yellow('Found existing .vscode/settings.json. Merging SEAD settings...'));
      } catch {
    ...
    if (configChoice === 'defaults') {
      ...
      console.log(chalk.green('✓ Using recommended SEAD defaults for Github Copilot settings'));
    } else {
      ...
      seadSettings = {
        'chat.agent.enabled': true, // Always enabled - required for SEAD agents
    ...
    const mergedSettings = { ...seadSettings, ...existingSettings };
    ...
    for (const [key, value] of Object.entries(seadSettings)) {
