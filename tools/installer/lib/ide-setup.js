...
class IdeSetup extends BaseIdeSetup {
  ...
  async setupCodex(installDir, selectedAgent, options) {
    ...
    section += `- Reinstall BMAD core and regenerate AGENTS.md: \`npx sead-method install -f -i codex\`\n`;
    ...
    try {
      ...
      if (await fileManager.pathExists(pkgPath)) {
        ...
        if (!updated['bmad:refresh']) updated['bmad:refresh'] = 'sead-method install -f -i codex';
        if (!updated['bmad:list']) updated['bmad:list'] = 'sead-method list:agents';
        if (!updated['bmad:validate']) updated['bmad:validate'] = 'sead-method validate';
    ...
    try {
      ...
      const ignoreLines = ['# BMAD (local only)', '.sead-core/', '.bmad-*/'];
      ...
      if (options.webEnabled) {
        if (exists) {
          ...
          const updated = gi
            .split(/\r?\n/)
            .filter((l) => !/^\s*\.sead-core\/?\s*$/.test(l) && !/^\s*\.bmad-\*\/?\s*$/.test(l))
            .join('\n');
          if (updated !== gi) {
