...
class Installer {
  ...
  async install(config) {
    ...
    try {
      ...
      if (path.basename(installDir) === '.sead-core') {
        // If user points directly to .sead-core, treat its parent as the project root
  ...
  async detectInstallationState(installDir) {
    ...
    if (files.length > 0) {
      // Directory has other files, but no SEAD installation.
  ...
  async performFreshInstall(config, installDir, spinner, options = {}) {
    spinner.text = 'Installing SEAD Method...';
    ...
    switch (config.installType) {
      case 'full': {
        ...
        // Copy common/ items to .sead-core
        spinner.text = 'Copying common utilities...';
        await this.copyCommonItems(installDir, '.sead-core', spinner);

        // Copy documentation files from docs/ to .sead-core
        spinner.text = 'Copying documentation files...';
        await this.copyDocsItems(installDir, '.sead-core', spinner);
        ...
        files = foundFiles.map((file) => path.join('.sead-core', file));
      ...
      case 'single-agent': {
        ...
        files.push(`.sead-core/agents/${config.agent}.md`);

        // Copy dependencies
        const { all: dependencies } = await resourceLocator.getAgentDependencies(config.agent);
        const sourceBase = resourceLocator.getSeadCorePath();
        ...
        for (const dep of dependencies) {
          ...
          if (dep.includes('*')) {
            ...
            const copiedFiles = await fileManager.copyGlobPattern(
              dep.replace('.sead-core/', ''),
              sourceBase,
              path.join(installDir, '.sead-core'),
              '.sead-core',
            );
            files.push(...copiedFiles.map((f) => `.sead-core/${f}`));
          } else {
            // Handle single files with {root} replacement if needed
            const sourcePath = path.join(sourceBase, dep.replace('.sead-core/', ''));
            ...
            success = await (needsRootReplacement
              ? fileManager.copyFileWithRootReplacement(sourcePath, destinationPath, '.sead-core')
        ...
        // Copy common/ items to .sead-core
        spinner.text = 'Copying common utilities...';
        const commonFiles = await this.copyCommonItems(installDir, '.sead-core', spinner);
        files.push(...commonFiles);

        // Copy documentation files from docs/ to .sead-core
        spinner.text = 'Copying documentation files...';
        const documentFiles = await this.copyDocsItems(installDir, '.sead-core', spinner);
      ...
      case 'expansion-only': {
        // Expansion-only installation - DO NOT create .sead-core
  ...
  async handleExistingV4Installation(config, installDir, state, spinner) {
    ...
    console.log(chalk.yellow('\n🔍 Found existing SEAD v4 installation'));
    ...
    if (versionCompare < 0) {
      console.log(chalk.cyan('\n⬆️  Upgrade available for SEAD core'));
      choices.push({
        name: `Upgrade SEAD core (v${currentVersion} → v${newVersion})`,
        value: 'upgrade',
      });
      ...
      choices.push({
        name: `Force reinstall SEAD core (v${currentVersion} - reinstall)`,
        value: 'reinstall',
      });
    } else {
      console.log(chalk.yellow('\n⬇️  Installed version is newer than available'));
      choices.push({
        name: `Downgrade SEAD core (v${currentVersion} → v${newVersion})`,
        value: 'reinstall',
      });
    }
  ...
  async handleV3Installation(config, installDir, state, spinner) {
    ...
    console.log(chalk.yellow('\n🔍 Found SEAD v3 installation (bmad-agent/ directory)'));
    console.log(`   Directory: ${installDir}`);
  ...
  async handleUnknownInstallation(config, installDir, state, spinner) {
    ...
    if (state.hasBmadCore) {
      console.log('   Found: .sead-core directory (but no manifest)');
    }
  ...
  async performRepair(config, installDir, manifest, integrity, spinner) {
    ...
    try {
      ...
      for (const file of filesToRestore) {
        ...
        const relativePath = file.replace('.sead-core/', '');
        ...
        if (await fileManager.pathExists(commonSourcePath)) {
          ...
          const updatedContent = content.replaceAll('{root}', '.sead-core');
  ...
  showSuccessMessage(config, installDir, options = {}) {
    console.log(chalk.green('\n✓ SEAD Method installed successfully!\n'));
    ...
    if (ides.length > 0) {
      for (const ide of ides) {
        ...
        if (ideConfig?.instructions) {
          console.log(chalk.bold(`To use SEAD agents in ${ideConfig.name}:`));
    ...
    if (config.installType !== 'expansion-only') {
      console.log(chalk.green('✓ .sead-core framework installed with all agents and workflows'));
    }
    ...
    if (config.installType === 'single-agent') {
      console.log(chalk.dim('\nNeed other agents? Run: npx sead-method install --agent=<name>'));
      console.log(chalk.dim('Need everything? Run: npx sead-method install --full'));
    }
    ...
    console.log(
      chalk.red.bold(
        '\n📖 IMPORTANT: Please read the user guide at docs/user-guide.md (also installed at .sead-core/user-guide.md)',
      ),
    );
    console.log(
      chalk.red(
        'This guide contains essential information about the SEAD workflow and how to use the agents effectively.',
      ),
    );
  }
  ...
  async update() {
    ...
    console.log(chalk.red('No SEAD installation found.'));
  }

  async listAgents() {
    const agents = await resourceLocator.getAvailableAgents();

    console.log(chalk.bold('\nAvailable SEAD Agents:\n'));

    for (const agent of agents) {
      console.log(chalk.cyan(`  ${agent.id.padEnd(20)}`), agent.description);
    }

    console.log(chalk.dim('\nInstall with: npx sead-method install --agent=<id>\n'));
  }

  async listExpansionPacks() {
    const expansionPacks = await resourceLocator.getExpansionPacks();

    console.log(chalk.bold('\nAvailable SEAD Expansion Packs:\n'));
    ...
    console.log(chalk.dim('Install with: npx sead-method install --full --expansion-packs <id>\n'));
  }
  ...
  async showStatus() {
    ...
    if (!installDir) {
      console.log(chalk.yellow('No SEAD installation found in current directory tree'));
      return;
    }
              ...
              if (!(await fileManager.pathExists(expansionDepPath))) {
                ...
                if (await fileManager.pathExists(sourceDepPath)) {
                  ...
                  const coreDepPath = path.join(
                    resourceLocator.getSeadCorePath(),
                    depType,
                    depFileName,
                  );
  ...
  async installWebBundles(webBundlesDirectory, config, spinner) {
    try {
      // Find the dist directory in the SEAD installation
  ...
  async detectExpansionPacks(installDir) {
    ...
    const dotFolders = glob.sync('.*', {
      cwd: installDir,
      ignore: ['.git', '.git/**', '.sead-core', '.sead-core/**'],
    });
  ...
  async findInstallation() {
    // Look for .sead-core in current directory or parent directories
    let currentDir = process.cwd();

    while (currentDir !== path.dirname(currentDir)) {
      const bmadDir = path.join(currentDir, '.sead-core');
      const manifestPath = path.join(bmadDir, 'install-manifest.yaml');

      if (await fileManager.pathExists(manifestPath)) {
        return currentDir; // Return parent directory, not .sead-core itself
      }

      currentDir = path.dirname(currentDir);
    }

    // Also check if we're inside a .sead-core directory
    if (path.basename(process.cwd()) === '.sead-core') {
